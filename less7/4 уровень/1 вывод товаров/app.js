...
let divp = document.querySelector('.products');

btn.forEach(function(clickbtn) {
...
function clickHandler(event) {
    ...
    divp.innerHTML = "";
    console.dir(event); //хотела посмотреть target'ы
...
function showCategory(category) {
    ...
    //этот вариант короче
    var arrCategory = products[category];
    for (var i = 0; i < arrCategory.length; i++){
      divp.insertAdjacentHTML("afterbegin", getProductMarkup(arrCategory[i]));
    }
  ...
  *п
   const categoryProducts = products[category];
        ...
        divp.insertAdjacentHTML("afterbegin", markupToProductsDiv);*/
    }
